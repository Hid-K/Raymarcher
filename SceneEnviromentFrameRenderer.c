...
void render_frame_of_enviroment(RGB * frame_buffer,
                                ...
                                SceneEnviroment * enviroment)
{
    ...
    for(size_t x = 0; x < buffer_width; ++x)
    {
        for(size_t y = 0; y < buffer_height; ++y)
        {
            ...
            // ray_direction = add(ray_direction, multiply(enviroment->main_camera.direction, fov));
...
RGB castRay(Vec3 origin, Vec3 direction, SceneEnviroment * enviroment)
{
    ...
    currientRay.origin = enviroment->main_camera.origin;
